...
const KeyboardProvider = ({
  ...
  const setPressed = useCallback((keyCode: string) => {
    ...
    previewTimeoutRef.current = setTimeout(() => {
      setLastPressedKey((currentKey) => (currentKey === keyCode ? null : currentKey));
    }, 1000);
  }, []);
...
const KeystrokePreview = () => {
  ...
  const displayKey = lastPressedKey ? getKeyDisplayLabel(lastPressedKey) : null;
  const isPressed = Boolean(lastPressedKey && pressedKeys.has(lastPressedKey));

  return (
    <div
      data-key-preview-layer
      className="pointer-events-none absolute inset-x-0 bottom-[calc(100%+0.5rem)] flex items-center justify-center pt-2"
      aria-live="polite"
      aria-atomic="true"
    >
      <AnimatePresence mode="popLayout">
        {displayKey && (
          <motion.div
            ...
            className="relative flex items-center justify-center font-mono text-2xl font-black text-neutral-700 dark:text-neutral-200"
          >
            <motion.span
